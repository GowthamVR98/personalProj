...
if(isset($_GET['designationId'])){
    ...
    $id = $SafeCrypto->decrypt(trim($_GET['designationId']));
    $exists = $dbobj->selectData($dbh, TBL_PRIFIX.'designation_master', array('designation'), 'designation_id = ?', array($id));
    if($exists['total'] > 0){
        $data = $exists['values'][0]['designation'];
    }else{
        $_SESSION['error'] = 'No record found.';
        header('location:alumni-add-designation');
        exit(0);
    }
}else{
...
if(isset($_POST['btnskill'])){
    $checkExists = $dbobj->check_exist($dbh, TBL_PRIFIX.'designation_master',array('industry_id'), 'designation = ?', array(trim($_POST['txtSkill'])),$id,true);
    if($checkExists){
        $_SESSION['error'] = 'Industry already exist.';
        header('location:alumni-edit-designation.php?designationId='.$SafeCrypto->encrypt($id));
        exit(0);
    }else{
        $update = $dbobj->UpdatetData($dbh, TBL_PRIFIX.'designation_master', 'designation = ?', 'designation_id = ?', array(trim($_POST['txtSkill']),$id));
        if($update){
                        ...
                        <form id="form_validation" method="POST">
                            <div class="header">
                                ...
                                <ul class="header-dropdown m-r--5">
                                    <li class="dropdown">
                                        <button class="btn btn-warning waves-effect" onclick="location.href='alumni-add-designation'" type="button">Back</button>
