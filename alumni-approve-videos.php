...
$loginSessionId = $dbobj->getSessionId();
    if(isset($_POST['btnReject'])){
        $update = $dbobj->UpdatetData($dbh, TBL_PRIFIX.'video_details', 'reject_reason = ?, approval_status = ?, modified_date = ?,modified_by = ?,modified_ip = ?', 'video_id = ?', array(trim($_POST['txtRejectReason']),'rejected',CURRENT_DT,$loginSessionId,$_SERVER['REMOTE_ADDR'],$_POST['hiddenId']));
        if($update){
            $_SESSION['success'] = 'Video has been rejected successfully.';
           
        }else{
            $_SESSION['warning'] = 'Somthing went to wrong, Please try again later.';
        }
         header('location:alumni-approve-videos');
        exit(0);
    }
?>
<!DOCTYPE html>
...
<body class="theme-red">
    ...
    <script>
        $(document).ready(function(){
            ...
            $.ajax({
                type:'POST',
                url:'ajax/ajax-approve-videos.php',
                data:'pageno='+pageno,
